...
export default function CaregiverGuideTemplate() {
  const [matchIndex] = useRoute("/guide-to-being-a-caregiver");
  const [matchSub, params] = useRoute("/guide-to-being-a-caregiver/:slug");

  const slug = matchIndex ? "" : (params?.slug ?? "");
  const page = CAREGIVER_PAGES.find((p) => p.slug === slug);

  // CMS SEO override
  const cmsSlug = slug === "" ? "caregiver-guide" : `caregiver-${slug}`;
  useCMSSEO({
    contentType: "page",
    slug: cmsSlug,
    title: page ? `${page.title} | MedicareFAQ` : "Caregiver Guide | MedicareFAQ",
    description: page?.heroDescription ?? "",
    canonical: `https://www.medicarefaq.com/guide-to-being-a-caregiver${slug ? `/${slug}` : ""}`,
  });

  if (!page) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        <h1 className="text-2xl font-bold text-navy-900 mb-4">Page Not Found</h1>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      {/* SEO is handled by useCMSSEO hook above */}
      ...
      <section className="bg-[#1B3A5C] text-white py-12 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <nav className="flex items-center gap-1 text-sm text-blue-200 mb-6">
            ...
            {slug !== "" ? (
              <>
                <Link href="/guide-to-being-a-caregiver" className="hover:text-white transition-colors">
                  Caregiver Guide
                </Link>
                <ChevronRight className="w-3 h-3" />
              ...
              <span className="text-white">Caregiver Guide</span>
            )}
          </nav>
              ...
              <section key={section.id} id={section.id} className="mb-10">
                ...
                <div
                  className="prose prose-gray max-w-none text-gray-700 leading-relaxed
                    ...
                    prose-table:text-sm prose-th:bg-gray-50 prose-th:font-semibold prose-td:border prose-td:border-gray-200 prose-th:border prose-th:border-gray-200 prose-th:p-2 prose-td:p-2"
